...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const zh = {
      ...
      overridden: '已覆盖',
      reset: '恢复默认',
      toggleInstantDescribe: '即时本地翻译',
      hintInstantDescribe: '开启后（需同时开启本地 Ollama），图片轮第一轮直接用本地模型识别图片块——模型第一轮即看懂，不必先调用识图工具；一次贴多张图时并发识别（上限 3，本地推理受显存限制），单张失败自动跳过、不影响其余；整体失败则回退为工具提示标记。会话日志仍保留原图。',
      groupLocalOllama: '本地视觉（并入自 dsh-vision）',
      localVisionOff: '未开启',
      localVisionOn: '已开启',
      ...
      hintLocalLmStudio: '与 Ollama 同层级的本地后端：LM Studio 的 OpenAI 兼容端点（默认 http://localhost:1234/v1）。模型名仅为占位——LM Studio 使用当前加载的模型，填什么都行。启用后 local-lmstudio 排在 local-ollama 之后、云链之前；未运行时自动跳过降级。默认关闭。',
      localDescribeStyle: '本地识别输出风格',
      localDescribeStylePlain: '平铺描述',
      localDescribeStyleStructured: '结构化（【初步判断】格式）',
      hintLocalDescribeStyle: '仅影响本地识别（即时翻译 / 截图识别）的输出格式；结构化对截图分析（GUI / 文档 / 聊天记录）质量更高。',
    ...
    const en = {
      ...
      reset: 'Reset',
      toggleInstantDescribe: 'Instant local translation',
      hintInstantDescribe: 'When on (and local Ollama is enabled), image blocks are recognized by the local model on the first model step — the model understands the image immediately without calling a vision tool first. Multiple images are recognized concurrently (up to 3 — local inference is VRAM-bound); a failing image is skipped without affecting the rest; total failure falls back to the tool-hint marker. The session log keeps the original image.',
      groupLocalOllama: 'Local vision (merged from dsh-vision)',
      localVisionOff: 'Off',
      localVisionOn: 'On',
      ...
      hintLocalLmStudio: 'A local backend on the same level as Ollama: LM Studio\'s OpenAI-compatible endpoint (default http://localhost:1234/v1). The model name is only a placeholder — LM Studio uses the currently loaded model, any value works. When enabled, local-lmstudio sits after local-ollama and before the cloud chain; when LM Studio is down it is skipped automatically. Off by default.',
      ...
      hintLocalDescribeStyle: 'Affects only local recognition output (instant translation / screenshot identify); structured is better for screenshot analysis (GUI / documents / chat logs).',
    ...
    const CSS =
      ...
      '.vr-local-body{display:flex;flex-direction:column;gap:8px}' +
    ...
    function VisionRouterCard(props) {
      ...
      const format = (key) => {
        ...
        if (key === 'localOllama') {
          ...
          return {
            ...
            model:
              typeof o.model === 'string' && o.model !== '' ? o.model : 'qwen2.5vl',
            temperature: typeof o.temperature === 'number' ? o.temperature : 0.5,
            top_p: typeof o.top_p === 'number' ? o.top_p : 0.8,
          }
        }
        if (key === 'localLmStudio') {
          ...
          return {
            ...
            model:
              typeof o.model === 'string' && o.model !== '' ? o.model : 'local-model',
            temperature: typeof o.temperature === 'number' ? o.temperature : 0.5,
            top_p: typeof o.top_p === 'number' ? o.top_p : 0.8,
          }
        }
      ...
      const parse = (key, text) => {
        ...
        if (key === 'localOllama') {
          const o = text && typeof text === 'object' ? text : {}
          const baseURL =
          ...
          const temperature =
            typeof o.temperature === 'number' && Number.isFinite(o.temperature)
              ...
              : undefined
          const top_p =
            typeof o.top_p === 'number' && Number.isFinite(o.top_p)
              ? Math.min(1, Math.max(0, o.top_p))
              : undefined
          return {
            value: {
              enabled: o.enabled === true,
      ...
      // 折叠状态下的紧凑摘要：不展开分组也能一眼看出本地视觉是否生效
      // （未开启 / 已开启 · Ollama <model> 或 LM Studio <model> · 输出风格 ·
      // 即时识别）。
      const localVisionSummary = () => {
        ...
        if (ollama.enabled === true) {
          parts.push(`Ollama ${ollama.model || 'qwen2.5vl'}`)
        } else if (lmStudio.enabled === true) {
          parts.push(`LM Studio ${lmStudio.model || 'local-model'}`)
        }
        parts.push(
          format('localDescribeStyle') === 'structured'
            ? t('localVisionStyleStructuredShort')
            : t('localVisionStylePlainShort'),
        )
        if (format('instantDescribe') === true) parts.push(t('localVisionInstantShort'))
        return parts.join(' · ')
      }
      ...
      const localOllamaEditor = () => {
        ...
        return h('div', { className: 'vr-field' },
          ...
          h('div', { className: 'vr-local-row' },
            ...
            h('input', {
              className: 'vr-input', value: value.baseURL, disabled: !writable,
          ...
          h('div', { className: 'vr-local-row' },
            ...
            h('input', {
              className: 'vr-input', value: value.model, disabled: !writable,
          ...
          h('div', { className: 'vr-local-row vr-local-row-pair' },
            ...
            h('input', {
              ...
              disabled: !writable, placeholder: '留空=服务端默认；建议 0.5', type: 'number', step: '0.1', min: '0', max: '2',
            ...
            h('input', {
              ...
              disabled: !writable, placeholder: '留空=服务端默认；建议 0.8', type: 'number', step: '0.05', min: '0', max: '1',
              onChange: (event) => update({
      ...
      const localLmStudioEditor = () => {
        ...
        return h('div', { className: 'vr-field' },
          ...
          h('div', { className: 'vr-local-row' },
            ...
            h('input', {
              className: 'vr-input', value: value.model, disabled: !writable,
              placeholder: 'local-model（占位，LM Studio 用当前加载的模型）',
              onChange: (event) => update({ model: event.target.value }),
            }),
          ),
          h('div', { className: 'vr-local-row vr-local-row-pair' },
      ...
      // dsh-vision 并入：识别输出风格（plain / structured 单选）。
      const describeStyleEditor = () => {
        const current = format('localDescribeStyle')
        return h('div', { className: 'vr-field' },
          h('div', { className: 'vr-field-head' },
            h('label', { className: 'vr-label' }, t('localDescribeStyle')),
            overriddenBadge('localDescribeStyle'),
          ),
          h('div', { className: 'vr-local-row' },
            ['plain', 'structured'].map((style) =>
              h('label', { className: 'vr-label vr-local-style' },
                h('input', {
                  type: 'radio', name: 'localDescribeStyle', checked: current === style,
                  disabled: !writable,
                  onChange: () => setDraft('localDescribeStyle', style),
                }),
                h('span', {}, t(style === 'plain' ? 'localDescribeStylePlain' : 'localDescribeStyleStructured')),
              ),
            ),
          ),
          h('p', { className: 'vr-hint' }, t('hintLocalDescribeStyle')),
        )
      }
      const finishGuide = () => {
        finishVisionSettingsGuide()
      ...
      return h('li', { className: 'vr-card' + (open ? ' vr-card-open' : '') },
        ...
        open
          ? h('div', { className: 'vr-body' },
              ...
              stealthNotice(),
              // dsh-vision 并入：本地视觉分组（即时翻译 + Ollama + LM Studio + 风格）
              // 默认折叠成一行（标题 + 状态摘要），点击展开编辑——分组里字段
              // 较多，收起来让设置卡更清爽。
