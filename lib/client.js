  ...
  factory: (require) => {
    ...
    function VisionRouterCard(props) {
      ...
      const format = (key) => {
        ...
        if (key === 'extraVisionModels') return Array.isArray(value) ? value.join('\n') : ''
      ...
      const parse = (key, text) => {
        ...
        if (key === 'extraVisionModels') {
          // The dropdown editor drafts an array of selected "provider/model"
          // entries; the free-text fallback (no catalog / no hidden models)
          // drafts a string. Accept both.
          const list = Array.isArray(text)
            ? text.map((entry) => String(entry ?? '').trim()).filter((entry) => entry !== '')
            : String(text ?? '')
                .split(/[\n,]+/)
      ...
      const extraVisionModelsEditor = () => {
        ...
        const selected = (Array.isArray(raw) ? raw : []).map((entry) => String(entry).trim())
        ...
        const update = (index, next) => {
          const list = rows.map((row, i) => {
            const r = i === index ? next : row
            return r.provider !== '' && r.model !== '' ? `${r.provider}/${r.model}` : ''
          })
          setDraft('extraVisionModels', list)
        }
        const removeRow = (index) => {
          const list = rows
            .map((row, i) => (i === index ? { provider: '', model: '' } : row))
        ...
        return h('div', { className: 'vr-field' },
          ...
          rows.map((row, index) =>
            h('div', { className: 'vr-chain-row', key: index },
              h('select', {
                ...
                value: hiddenProviders.includes(row.provider) ? row.provider : '',
                disabled: !writable,
                onChange: (event) => update(index, { provider: event.target.value, model: '' }),
              },
                h('option', { value: '' }, t('selectProvider')),
                hiddenProviders.map((provider) => h('option', { value: provider, key: provider }, provider)),
              ),
              h('select', {
                className: 'vr-input vr-select',
                value: row.model,
                ...
                (hiddenModelsOf.get(row.provider) ?? []).map((model) => {
          ...
          h('button', {
            ...
            onClick: () => setDraft('extraVisionModels', [...selected.filter((entry) => entry !== ''), '']),
          }, t('addFallback')),
