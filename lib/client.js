...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const zh = {
      ...
      hintVisionContext:
        '同一张图在同一会话里再次识图时，把上一轮识别结果自动附进新的提示词（零额外视觉调用、零延迟）。' +
    ...
    const en = {
      ...
      toggleVisionContext: 'Continuous vision context',
      ...
      hintVisionContext:
        'On repeated vision calls for the same image in one session, the previous result is appended to the next ' +
        'prompt as context (zero extra vision calls, zero latency). Off by default — the free endpoint is rate-limited; ' +
        'enable it on paid vision chains.',
      hintStealth:
    ...
    const ADVANCED_TOGGLE_KEYS = ['reverseRouting', 'rewriteImages', 'downscale', 'cache', 'freeFallback', 'visionContext']
    ...
    const LABEL_KEY = {
      ...
      visionContext: 'toggleVisionContext',
    ...
    const HINT_KEY = {
      ...
      visionContext: 'hintVisionContext',
      stealth: 'hintStealth',
